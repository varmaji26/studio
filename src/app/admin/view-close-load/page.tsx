...
export default function ViewCloseLoadPage() {
  ...
  useEffect(() => {
    ...
    const loadData: GameLoad[] = games.map(game => {
      const gameBids = allCloseBids.filter(bid => bid.gameId === game.id);
  ...
  useEffect(() => {
      ...
      const gameBids = allCloseBids.filter(bid => bid.gameId === selectedGame.id);
  ...
  return (
    <div className="flex-1 space-y-6">
      <div className="space-y-6">
        <div>
            <h1 className="text-3xl font-bold">View Close Load</h1>
            <p className="text-muted-foreground">Click on a game to see its live bidding details for the selected date's Close session below.</p>
            <div className="pt-4">
               <Popover>
        ...
        {selectedGame && liveBiddingDetails.map((details) => (
            ...
            <Card key={details.betType} className="bg-card/80 border-white/10 shadow-lg">
              <CardHeader>
                  <CardTitle>Live Bidding - {details.gameName} - Close {details.betType} - Total Load ₹{details.totalLoad.toFixed(2)}</CardTitle>
              </CardHeader>
              <CardContent>
                  <div className={cn(
                      "grid gap-4",
