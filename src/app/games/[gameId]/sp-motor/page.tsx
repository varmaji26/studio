...
import { collection, runTransaction, doc, serverTimestamp, increment, DocumentData } from 'firebase/firestore';
...
export default function SpMotorPage() {
  ...
  const { isOpenSessionAllowed, isCloseSessionAllowed, isBettingAllowed } = useMemo(() => {
    ...
    const bettingAllowed = (session === 'Open' && openAllowed) || (session === 'Close' && closeAllowed);

    return {
        isOpenSessionAllowed: openAllowed,
        isCloseSessionAllowed: closeAllowed,
  ...
  const handleGenerate = (data: FormValues) => {
    ...
    const generatedPanas = combinations.map(c => c.sort().join(''));
    ...
    const newBids: BidItem[] = generatedPanas.map(p => ({ number: p, amount: points }));

    setSubmittedBids(prev => [...prev, ...newBids]);
    toast({ title: "Bids Generated", description: `${newBids.length} SP Motor bids have been added to your list.` });
    form.reset({
                            ...
                            <div className="space-y-1 p-1">
                                {submittedBids.map((bid, index) => (
                                    <div key={index} className="flex justify-between items-center bg-slate-800 p-1 px-3 rounded-md border border-white/5 text-[10px] animate-in fade-in-0">
                                        <p>Pana: <span className="font-bold">{bid.number}</span></p>
          ...
          <CardFooter className="fixed bottom-0 left-0 right-0 max-w-2xl mx-auto bg-background/80 backdrop-blur-sm border-t border-border p-4 flex items-center justify-between gap-4 z-10">
            <div className="flex items-center gap-4">
                <div className="flex flex-col text-left">
                  <span className="text-xs text-muted-foreground uppercase font-bold">Bids</span>
