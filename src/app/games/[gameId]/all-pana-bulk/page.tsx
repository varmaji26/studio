...
export default function AllPanaBulkPage() {
  ...
  const handlePlaceBet = async () => {
    ...
    const allPanas = selectedPanas;

    const singlePanas: string[] = [];
    const doublePanas: string[] = [];
    const triplePanas: string[] = [];
    const invalidPanas: string[] = [];

    allPanas.forEach(pana => {
        if (!/^\d{3}$/.test(pana)) {
            invalidPanas.push(pana);
            return;
        }
        const digits = new Set(pana.split(''));
        if (digits.size === 3) {
            singlePanas.push(pana);
        } else if (digits.size === 2) {
            doublePanas.push(pana);
        } else if (digits.size === 1) {
            triplePanas.push(pana);
        } else {
            invalidPanas.push(pana);
        }
    });
    ...
    if (invalidPanas.length > 0) {
        toast({ variant: 'destructive', title: 'Invalid Numbers', description: `Some numbers are not 3 digits: ${invalidPanas.join(', ')}` });
        return;
    }
    
    const betsToPlace = [];
    if (singlePanas.length > 0) betsToPlace.push({ type: 'Single Pana', numbers: singlePanas });
    if (doublePanas.length > 0) betsToPlace.push({ type: 'Double Pana', numbers: doublePanas });
    if (triplePanas.length > 0) betsToPlace.push({ type: 'Triple Pana', numbers: triplePanas });

    if (betsToPlace.length === 0) {
      toast({ variant: 'destructive', title: 'Error', description: 'Please enter at least one valid Pana number.' });
      return;
    }

    const parsedAmount = parseInt(amount, 10);
    if (!amount || parsedAmount <= 0) {
      toast({ variant: 'destructive', title: 'Error', description: 'Please enter a valid bet amount per number.' });
      return;
    }
    ...
    const totalBetAmount = allPanas.length * parsedAmount;
    ...
    try {
        await runTransaction(db, async (transaction) => {
            ...
            if (totalUserBalance < totalBetAmount) {
            ...
            if (currentBalance >= totalBetAmount) {
                amountFromReal = totalBetAmount;
            } else {
                amountFromReal = currentBalance;
                amountFromBonus = totalBetAmount - currentBalance;
            }
            ...
            for (const bet of betsToPlace) {
                 const betAmountForType = bet.numbers.length * parsedAmount;
                 transaction.set(doc(bidsCollectionRef), {
                    ...
                    session,
                    numbers: bet.numbers,
                    amountPerBet: parsedAmount,
                    totalAmount: betAmountForType,
                    status: 'running',
        ...
        toast({
            ...
            description: `Your bets totaling ₹${totalBetAmount} have been placed for ${game?.name}.`,
    ...
    <div className="space-y-4">
        <Card className="bg-background/80 border-white/10">
            <CardHeader className="p-4">
                <CardTitle className="text-base">Enter SP/DP/TP Numbers (Bulk):</CardTitle>
            </CardHeader>
            <CardContent className="p-4 pt-0">
                <Textarea 
                    placeholder="Enter numbers separated by space or new line, e.g., 123 445 777"
        ...
        <Card className="bg-background/80 border-white/10">
            ...
            <CardContent className="space-y-1 text-xs p-4 pt-0">
                ...
                <div className="flex justify-between">
                    ...
                    <span className="font-semibold">All Pana (Bulk)</span>
                </div>
                <div className="flex justify-between">
