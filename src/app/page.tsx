  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="flex flex-col gap-4 p-4 pb-28">
              ...
              <div className="grid grid-cols-2 gap-3">
                {games.map((game) => (
                  <div key={game.id} className="flex flex-col items-center justify-center bg-[#34a387] p-1 rounded-lg border border-black text-center">
                    ...
                    <span className="text-xs font-bold text-black [text-shadow:1px_1px_2px_#fff]">{formatGameResult(game, true)}</span>
                    <span className="text-xs text-white/70">({formatTime(game.closeTime)})</span>
                  </div>
                ))}
              </div>
            ) : (
        ...
        <Card className="bg-[#3D1C27] border-white/10 shadow-lg">
            <CardHeader>
