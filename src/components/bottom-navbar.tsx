...
export function BottomNavbar({ settings }: BottomNavbarProps) {
    ...
    const navItems = [
        { label: 'My Bids', action: () => router.push('/bids-history'), color: 'bg-yellow-500', icon: <Trophy className="h-5 w-5" style={{ filter: 'drop-shadow(1px 1px 2px #000)' }} /> },
        { label: 'Passbook', action: () => router.push('/payment-history'), color: 'bg-green-500', icon: <BookText className="h-5 w-5" style={{ filter: 'drop-shadow(1px 1px 2px #000)' }} /> },
